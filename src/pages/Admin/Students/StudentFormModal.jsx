...
const STUDY_YEAR_OPTIONS = Array.from({ length: 10 }, (_, i) => i + 1);
...
export default function StudentFormModal({ student, departments, saving, onSubmit, onClose }) {
  ...
  const validate = () => {
    ...
    if (!values.study_year) next.study_year = "الرجاء اختيار السنة الدراسية";
  ...
  const handleSubmit = (event) => {
    ...
    const payload = {
      ...
      study_year: Number(values.study_year),
    };
    if (isEdit) {
      ...
      <form id="student-form" onSubmit={handleSubmit} noValidate>
        <div className="form-grid">
          ...
          <div className="form-field">
            ...
            <select value={values.study_year} onChange={setField("study_year")}>
              <option value="">اختر السنة الدراسية</option>
              {STUDY_YEAR_OPTIONS.map((y) => (
